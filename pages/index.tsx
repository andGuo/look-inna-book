  ...
  return (
    <Layout title={"Home | Look-Inna-Book"}>
      <div className="flex flex-wrap flex-col items-center justify-around sm:my-4 sm:flex-row sm:items-start sm:justify-center">
        <div className="col-span-3 grid grid-cols-3 items-center bg-stone-300 p-2 m-4 lg:w-4/5 sm:w-2/3">
          {shownBooks.map((book) => (
            <div className="rounded-2xl drop-shadow-2xl bg-stone-600 m-3">
              <Link href={`/books/${book.isbn}`}>
                <div className="flex flex-col justify-center items-center p-4 max-h-fit">
                  ...
                  <div className="flex flex-col justify-center px-2 py-4 mx-2 text-2xl text-darkText text-center">
                    <div>{`${book.title}`} </div> <div>-</div> <div>{`$${book.msrp}`}</div>
                  </div>
                </div>
              </Link>
            </div>
